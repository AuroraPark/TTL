...
#### Q. 직책이 사원인 사람들의 평균연봉을 부서별로 구하고 연봉이 높은 순으로 정렬합니다!!!!!!!
...
#### Q4. 각 부서별로 직책이 사원인 직원들의 평균 급여를 계산해서 보여주시오
```
...
#### Q2. 각 부서별(dept_id)로 평균 급여(avg(salary))를 구하되 평균 급여가 2000 이상(avg(salary)>=2000)인 부서만 나타내시오
```
select dept_id,avg(salary) from s_emp
...
#### Q1. 각 부서별로 직책이 사원인 직원들에 대해서만 평균 급여를 구하시오
```
...
;
```
#### Q3. 각 부서내(dept_id)에서 몇명(count(*))의 직원이 근무하는지를 나타내시오
...
#### Q4. 각 부서별(dept_id)로 급여의 최소값(min(salary))과 최대값(max(salary))을 나타내시오 단, 최소값과 최대값이 같은 부서((min(salary))=(max(salary))는 출력하지 마시오.
```
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIxMzUzOTUxNTAsLTE3NjQyOTE0MCwtMj
AwNjQzNTUzNywtMzI3MzU4MjYyLC0xMDAyNjgwMTQ5LC0xOTE3
MTkyNTI2LC0xNDAzNTEwMzk3LDY4MTM2NTU4OSwtMTk0MzgwOD
I0OSw5NzI1NDg4MDAsLTI3NjA3MDkyLDE4MTA3NTE4OTgsMjA3
MzcwNzU0MywxNjEzMDM0MTc4LDIxMDA2OTk5NTUsLTY3MzQ5Nz
g4Myw1ODQ2OTIwMDMsLTE3ODk2Njc3NTNdfQ==
-->